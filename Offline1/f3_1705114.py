...
def sub_invbox(bitvector):
    ...
    for i in range(0, bitvector.length(), 8):
        index = bitvector[i: i+8].intValue()
        invsub_bitvector += BitVector(intVal=InvSbox[index], size=8)
...
def inv_mix_column(bitvector):
    ...
    for i in range(4):
        ...
        for j in range(4):
            temp = InvMixer[i][0].gf_multiply_modular(matrix[0][j], AES_modulus, 8)
            temp ^= InvMixer[i][1].gf_multiply_modular(matrix[1][j], AES_modulus, 8)
            temp ^= InvMixer[i][2].gf_multiply_modular(matrix[2][j], AES_modulus, 8)
            temp ^= InvMixer[i][3].gf_multiply_modular(matrix[3][j], AES_modulus, 8)
            result_matrix_row.append(temp)
